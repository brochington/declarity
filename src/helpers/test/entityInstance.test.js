...
describe('Entity Instance helpers', () => {
  describe('diffComponents()', () => {
    it('has correct diff output', () => {
      ...
      const diffContent = entityInstanceHelpers.diffComponents(
        oldContent,
        newContent
      );

      expect(diffContent.added).to.eql([{ key: '5' }]);
      expect(diffContent.removed).to.eql([
        { key: '2' },
        { key: '3' },
        { key: '4' },
      ]);
  ...
  describe('prepareChildContentForMounting()', () => {
    it('returns correct content', () => {
      ...
      const groomedContent = entityInstanceHelpers.prepareChildContentForMounting(
        childContent
      );
      ...
      expect(groomedContent.entityInstance.entityClass.name).to.equal(
        entityClass.name
      );
    });
  });
  ...
  describe('mountChildren()', () => {
    it('calls mount on children', () => {
      ...
      const childEntities = entityInstanceHelpers.mountChildren(
        childrenReadyForMounting
      );
